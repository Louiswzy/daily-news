...
today_str   = now.strftime("%Y%m%d")           # 20260515
today_label = now.strftime("%Y年%m月%d日")     # 2026年05月15日
month_key   = now.strftime("%Y%m")             # 202605
month_label = now.strftime("%Y年%m月")         # 2026年05月

ANTHROPIC_KEY       = os.environ["ANTHROPIC_API_KEY"]
NOTION_KEY          = os.environ["NOTION_API_KEY"]       # npt_xxx 或 token_v2 的值
PARENT_PAGE_ID      = os.environ["NOTION_PARENT_PAGE_ID"]  # Daily News 页面 ID

# npt_ 开头是 Personal access token，用 Bearer 方式
# token_v2 是 cookie，需要用 cookie 方式
_is_personal_token = NOTION_KEY.startswith("npt_")
_is_token_v2       = not _is_personal_token

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 1. 调用 Claude 生成新闻内容（JSON 格式）
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def generate_news():
...
每条新闻格式：
{
  "category": "ai" | "ecommerce" | "digital_marketing",
  "title": "新闻标题",
  "summary": "100字以内摘要",
  "url": "信息来源URL"
}
...
# 2. Notion API 工具函数
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# ── Notion 请求头（自动适配 Personal token 和 token_v2）
if _is_personal_token:
    NOTION_HEADERS = {
...
def notion_patch(url, data):
    r = requests.patch(url, headers=NOTION_HEADERS, json=data, timeout=30)
    r.raise_for_status()
    return r.json()
...
def text_block(content, bold=False, color=None):
    ann = {"bold": bold}
    t = {"content": content, "annotations": ann}
    if color:
        ann["color"] = color
    return t
...
def build_blocks(data):
    ...
    for region_key, (region_name, _) in REGION_META.items():
        ...
        for item in items:
            ...
            blocks.append(paragraph([("", False, None)]))  # 空行
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 4. 在 Daily News 主页找/创建月份 Toggle
#    再在 Toggle 内创建今日子页面
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def find_or_create_month_toggle(page_id, month_key, month_label):
    """在 Daily News 主页找 202605 的 toggle；找不到则新建。返回 toggle block id。"""
...
def create_news_subpage(parent_page_id, page_title, blocks):
    """在 Daily News 主页下创建子页面，返回页面 ID 和 URL。"""
    data = {
        ...
        "children": blocks[:100]  # Notion API 单次最多 100 blocks
    }
    result = notion_post("https://api.notion.com/v1/pages", data)
    return result["id"], result["url"]

def add_page_link_to_toggle(toggle_id, child_page_id, page_title):
    """在月份 Toggle 内追加一条指向子页面的 child_page 引用。"""
    notion_post(
...
def main():
    ...
    # Step 4: 在 Daily News 主页找/创建当月 Toggle，并添加链接
    print("→ 更新月份 Toggle...")
    toggle_id = find_or_create_month_toggle(PARENT_PAGE_ID, month_key, month_label)
    add_page_link_to_toggle(toggle_id, child_id, page_title)
    print("→ 链接已添加到 Toggle")
